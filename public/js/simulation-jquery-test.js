...
var agent2Traversal = [];
var humanTraversal = [];
var humanIndex = 0, agent1Index = 0, agent2Index = 0, agentNum = 1;
...
var viewRadius = 9;
var count = 0, waitCount = 0, seconds = 0, timeout, startTime;
...
var intervalCount = 0, half = 0, intervals = 10, duration = 999999;
...
$(document).ready(() => {
	...
	createMap(currentPath, function loop() {
		...
		if (!pause) {
			...
			// randomWalk(agent1);
			// randomWalk(agent2);
			if (agent2Index > agent2Traversal.length) {
				pause = true;
				console.log(humanExplored);
				console.log(tempAgent1Explored);
				console.log(tempAgent2Explored);
			} else {
				// human.loc = humanTraversal[humanIndex++];
				// agent1.loc = agent1Traversal[agent1Index++];
				agent2.loc = agent2Traversal[agent2Index++];
			} */

			if (humanIndex < humanTraversal.length) human.loc = humanTraversal[humanIndex++];
			if (agent1Index < agent1Traversal.length) agent1.loc = agent1Traversal[agent1Index++];
			if (agent2Index < agent2Traversal.length) agent2.loc = agent2Traversal[agent2Index++];

			if (humanIndex >= humanTraversal.length &&
				agent1Index >= agent1Traversal.length &&
				agent2Index >= agent2Traversal.length) {
					pause = true;
					console.log("True values...");
					console.log(humanExplored.size, tempAgent1Explored.size, tempAgent2Explored.size);
					console.log("Independent values...");
					console.log(
						difference(humanExplored, union(tempAgent1Explored, tempAgent2Explored)).size,
						difference(tempAgent1Explored, union(humanExplored, tempAgent2Explored)).size,
						difference(tempAgent2Explored, union(humanExplored, tempAgent1Explored)).size
					);
				}
...
function createMap(currentPath, cb) {
	...
	log = { agent1: [], agent2: [] };
	$log.empty();
	humanIndex = 0, agent1Index = 0, agent2Index = 0;
	...
	// ----SECTION1----
	$.getJSON('src/testData/testJSON.json', data => {
		$.each(data.section1.humanTraversal, (i, value) => {
			humanTraversal.push(value.loc);
		});
	});
	
	$.getJSON('src/testData/testJSON.json', data => {
		$.each(data.section1.agent1Traversal, (i, value) => {
			agent1Traversal.push(value.loc);
		});
	});
		
	$.getJSON('src/testData/testJSON.json', data => {
		$.each(data.section1.agent2Traversal, (i, value) => {
			agent2Traversal.push(value.loc);
		});
	});
	
	// ----SECTION2----
	$.getJSON('src/testData/testJSON.json', data => {
		$.each(data.section2.humanTraversal, (i, value) => {
			humanTraversal.push(value.loc);
		});
	});
	
	$.getJSON('src/testData/testJSON.json', data => {
		$.each(data.section2.agent1Traversal, (i, value) => {
			agent1Traversal.push(value.loc);
		});
	});
		
	$.getJSON('src/testData/testJSON.json', data => {
		$.each(data.section2.agent2Traversal, (i, value) => {
			agent2Traversal.push(value.loc);
		});
	});
	...
	$.getJSON(currentPath, data => {
		...
		human = { id: "human", loc: humanTraversal[humanIndex++]/* 131348 */, color: HUMAN_COLOR, dir: 1 };
		agent1 = { id: "agent1", loc: agent1Traversal[agent1Index++]/* 131320 *//* tempLoc1.y + tempLoc1.x*columns */, color: AGENT_COLOR, dir: 1, step: 1, stepsCovered: 0, minSteps: 7, maxSteps: 0 };
		agent2 = { id: "agent2", loc: agent2Traversal[agent2Index++]/* 141348 *//* tempLoc2.current[0][1] + tempLoc2.current[0][0]*columns */, color: AGENT_COLOR, dir: 1, step: 1, stepsCovered: 0, minSteps: 7, maxSteps: 0 };
