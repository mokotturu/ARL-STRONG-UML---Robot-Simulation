...
var intervalCount = 0, half = 0, intervals = 10, duration = 30000;
...
$(document).ready(() => {
	...
	createMap(currentPath, function loop() {
		...
		if (!pause) {
			...
			// moveAgent2(agent2);
			randomWalk(agent1);
			if (tempAgent1Explored.size >= emptyCells) {
				pause = true;
				console.log("done traversing: " + tempAgent1Explored.size);
			}
			// randomWalk(agent2);
		}
	});
...
function createMap(currentPath, cb) {
	...
	$.getJSON(currentPath, data => {
		...
		human = { id: "human", loc: 54818, color: HUMAN_COLOR, dir: 1 };
		agent1 = { id: "agent1", loc: 36818, color: AGENT_COLOR, dir: 1, step: 1, stepsCovered: 0, minSteps: 1, maxSteps: 0 };
